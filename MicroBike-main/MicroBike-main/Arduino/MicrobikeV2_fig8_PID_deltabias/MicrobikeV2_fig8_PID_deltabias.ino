...
float rollDesired_rad = .1;//.15;//atan(Unom*Unom/(9.81*Rturn));
...
void loop() {
  ...
  if(STRAIGHT||WAIT){
    ...
    rollDesired = .15*180/PI;//rollDesired_rad*180/PI;
  }
  else{
  ...
  intE = constrain(intE,-20.0/Ki,20.0/Ki);//with limit integral corrections to +/- 1 degree
...
myServo.write(steerCenterAngle - steer );
  }
  else{
