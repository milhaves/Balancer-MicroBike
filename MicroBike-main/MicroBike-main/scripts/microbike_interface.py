...
def doServo():
    global servo_cmd,ser,endSerialThread,servo_enable_cmd,tvec,feedbackvec,commandvec,file
    #initialize old time
    arduino_delay = .008
    timestr = time.strftime("%Y%m%d-%H%M%S")
    #f = open("data/data_"+timestr+".txt",'w')
    f = open("/Users/wenjia/Desktop/MicroBike/scripts/data/data_"+timestr+".txt",'w')
    ...
    while not endSerialThread:
        ...
        if 1:#tnow-lastsendtime>arduino_delay:     ### also happens super fast
            #print 'sent'
            lastsendtime = tnow
            # ser.write('!'.encode())
            ...
            if(len(line)>3):
                ...
                if(len(linesplit)>3):
                    ...
                    try:
                        ...
                        steerdeg = float(linesplit[1])
                        rollDesDeg = float(linesplit[2])
                        #convert second number to float:
                        feedbackval = float(linesplit[3])
...
portentry.insert(0,"/dev/cu.MICROBIKE_V2_1")
