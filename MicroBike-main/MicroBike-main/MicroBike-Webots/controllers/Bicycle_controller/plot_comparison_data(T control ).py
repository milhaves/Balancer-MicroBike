...
data=loadtxt("/Users/wenjia/Desktop/Bike/Bicycle_robot/controllers/Bicycle_controller/webots_data.txt",delimiter=",")
tdata = data[:,0]
speeddata = data[:,1]
rolldata = data[:,2]
steerdata = data[:,3]
...
for k in range(1,len(tmodel)):
	rollmodeldd=(1/h**2)*(g*h*rollmodel[k]-(a*h*V*sin(lamba)*k_roll*rollmodeld[k])/b-(sin(lamba)*k_roll*rollmodel[k]*(V**2*h+g*a*c))/b);
...
plot(tdata_range,rolldata_range-rolldata_range[0],'b',tmodel,rollmodel,'r')
